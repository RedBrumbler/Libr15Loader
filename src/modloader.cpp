...
char const* status_type(fs::file_type const type) {
  switch (type) {
...
#else
MAIN_LOCAL void modloader::preload() noexcept {}
#endif

#ifdef QUEST
MAIN_LOCAL void modloader::load(JNIEnv* env, char const* soDir) noexcept {
#else
MAIN_LOCAL void modloader::load(char const* soDir) noexcept {
#endif
  if (modloaderHandle == nullptr) {
...
#ifdef QUEST
MAIN_LOCAL void modloader::accept_r15_handle(JNIEnv* env, void* r15Handle) noexcept {
#else
MAIN_LOCAL void modloader::accept_r15_handle(void* r15Handle) noexcept {
#endif
  if (modloaderHandle == nullptr) {
    LOG_WARN("accept_r15_handle: Modloader not loaded!");
    return;
  }
  auto accept_r15_handle = reinterpret_cast<accept_r15_handle_t*>(dlsym(modloaderHandle, accept_r15_handleName.data()));
  if (accept_r15_handle == nullptr) {
    LOG_WARN("accept_r15_handle: Failed to find %s: %s", accept_r15_handleName.data(), dlerror());
    return;
  }
  LOG_VERBOSE("accept_r15_handle: Calling %s", accept_r15_handleName.data());
#ifdef QUEST
  accept_r15_handle(env, r15Handle);
#else
  accept_r15_handle(r15Handle);
#endif
  LOG_VERBOSE("accept_r15_handle: Init done");
}

#ifdef QUEST
MAIN_LOCAL void modloader::unload(JavaVM* vm) noexcept {
#else
MAIN_LOCAL void modloader::unload() noexcept {
#endif
